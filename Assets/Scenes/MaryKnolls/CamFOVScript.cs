...
public class CamFOVScript : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        // Set initial slider value to match CameraFOV
        if (MaxCameraFOVSlider != null)
        {
            MaxCameraFOVSlider.value = CameraFOV;  // Sets to 120f from public variable
            if (MaxCameraFOVText != null)
            {
                MaxCameraFOVText.text = CameraFOV.ToString("0.##");
            }
        }
        else
        {
            Debug.LogError("MaxCameraFOVSlider not assigned!");
        }
